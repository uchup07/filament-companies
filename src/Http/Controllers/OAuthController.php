...
namespace Uchup07\FilamentCompanies\Http\Controllers;
...
use Uchup07\FilamentCompanies\Enums\Feature;
use Uchup07\FilamentCompanies\FilamentCompanies;
use Uchup07\FilamentCompanies\Pages\User\Profile;
...
class OAuthController extends Controller
{
    ...
    protected function handleSignInNotFound(string $provider): RedirectResponse
    {
        return $this->redirectToWithError(
            __('filament-companies::companies.errors.signin_not_found', compact('provider')),
            $this->loginUrl,
        );
    }
    ...
    protected function handleCreateAccountOnFirstLogin(ProviderUser $providerAccount, string $provider): RedirectResponse | LoginResponse
    {
        if (FilamentCompanies::newUserModel()->where('email', $providerAccount->getEmail())->exists()) {
            return $this->redirectToWithError(__('filament-companies::companies.errors.already_connected', compact('provider')), $this->loginUrl);
        }
    ...
    {
        $title = __("filament-companies::companies.notifications.{$translationKey}.title");
        $body = __("filament-companies::companies.notifications.{$translationKey}.body", $translationParameters);
    ...
    protected function handleUserAlreadyRegistered(Authenticatable $user, ?ConnectedAccount $account, string $provider, ProviderUser $providerAccount): RedirectResponse | LoginResponse
    {
        ...
        return $this->redirectToWithError(
            __('filament-companies::companies.errors.already_associated_account', compact('provider')),
    ...
    protected function register(string $provider, ProviderUser $providerAccount): RedirectResponse | LoginResponse
    {
        ...
        if ($email === null) {
            return $this->redirectToWithError(__('filament-companies::companies.errors.no_email_with_account', compact('provider')), $this->registrationUrl);
        }

        if (FilamentCompanies::newUserModel()->where('email', $email)->exists()) {
            return $this->redirectToWithError(__('filament-companies::companies.errors.email_already_associated', compact('provider')), $this->registrationUrl);
        }

...
<div>
    @if (Gate::check('addCompanyEmployee', $company))
        ...
        <x-filament-companies::grid-section md="2">
            <x-slot name="title">
                {{ __('filament-companies::companies.grid_section_titles.add_company_employee') }}
            </x-slot>

            <x-slot name="description">
                {{ __('filament-companies::companies.grid_section_descriptions.add_company_employee') }}
            </x-slot>

            <x-filament::section>
                <x-filament-panels::form wire:submit="addCompanyEmployee">
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ __('filament-companies::companies.subheadings.companies.company_employee_manager') }}
                    </p>

                    <!-- Employee Email -->
                    <x-filament-forms::field-wrapper id="email" statePath="email" required="required" label="{{ __('filament-companies::companies.fields.email') }}">
                    ...
                    {{--@if (count($this->roles) > 0)
                        <x-filament-forms::field-wrapper id="role" statePath="role" required="required" label="{{ __('filament-companies::companies.labels.role') }}">
                    ...
                    <div class="text-left">
                        <x-filament::button type="submit">
                            {{ __('filament-companies::companies.buttons.add') }}
                        </x-filament::button>
                    </div>
    ...
    @if ($company->companyInvitations->isNotEmpty() && Gate::check('addCompanyEmployee', $company))
        ...
        <x-filament-companies::grid-section md="2">
            <x-slot name="title">
                {{ __('filament-companies::companies.action_section_titles.pending_company_invitations') }}
            </x-slot>

            <x-slot name="description">
                {{ __('filament-companies::companies.action_section_descriptions.pending_company_invitations') }}
            </x-slot>
                ...
                <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        <th colspan="3" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            {{ __('filament-companies::companies.fields.email') }}
                        </th>
                    </tr>
                    </thead>
    ...
    @if ($company->users->isNotEmpty())
        ...
        <x-filament-companies::grid-section md="2">
            <x-slot name="title">
                {{ __('filament-companies::companies.action_section_titles.company_employees') }}
            </x-slot>

            <x-slot name="description">
                {{ __('filament-companies::companies.action_section_descriptions.company_employees') }}
            </x-slot>
                ...
                <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        <th scope="col" colspan="3" class="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">
                            {{ __('filament-companies::companies.fields.name') }}
                        </th>
                    </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td colspan="1" class="px-6 py-4 whitespace-nowrap">
                                <div class="space-x-2 text-right">
                                    <!-- Manage Company Employee Role -->
                                    {{--@if (Gate::check('updateCompanyEmployee', $company) && Uchup07\FilamentCompanies\FilamentCompanies::hasRoles())
                                        <x-filament::button size="sm" outlined="true" color="primary" wire:click="manageRole('{{ $user->id }}')">
                                            {{ Uchup07\FilamentCompanies\FilamentCompanies::findRole($user->employeeship->role)->name }}
                                        </x-filament::button>
                                    @elseif (Uchup07\FilamentCompanies\FilamentCompanies::hasRoles())
                                    ...
                                    {{--@if ($this->user->id === $user->id)
                                        <x-filament::button size="sm" color="danger" wire:click="confirmLeavingCompany">
                                            {{ __('filament-companies::companies.buttons.leave') }}
                                        </x-filament::button>
    ...
    {{--<x-filament::modal id="currentlyManagingRole" icon="heroicon-o-shield-check" icon-color="primary" alignment="{{ $modals['alignment'] }}" footer-actions-alignment="{{ $modals['formActionsAlignment'] }}" width="{{ $modals['width'] }}">
        <x-slot name="heading">
            {{ __('filament-companies::companies.modal_titles.manage_role') }}
        </x-slot>
        ...
        <x-slot name="footerActions">
            ...
            <x-filament::button wire:click="updateRole">
                {{ __('filament-companies::companies.buttons.save') }}
            </x-filament::button>
        </x-slot>
    </x-filament::modal>--}}
    ...
    <x-filament::modal id="confirmingLeavingCompany" icon="heroicon-o-exclamation-triangle" icon-color="danger" alignment="{{ $modals['alignment'] }}" footer-actions-alignment="{{ $modals['formActionsAlignment'] }}" width="{{ $modals['width'] }}">
        <x-slot name="heading">
            {{ __('filament-companies::companies.modal_titles.leave_company') }}
        </x-slot>

        <x-slot name="description">
            {{ __('filament-companies::companies.modal_descriptions.leave_company') }}
        </x-slot>
    ...
    <x-filament::modal id="confirmingCompanyEmployeeRemoval" icon="heroicon-o-exclamation-triangle" icon-color="danger" alignment="{{ $modals['alignment'] }}" footer-actions-alignment="{{ $modals['formActionsAlignment'] }}" width="{{ $modals['width'] }}">
        <x-slot name="heading">
            {{ __('filament-companies::companies.modal_titles.remove_company_employee') }}
        </x-slot>

        <x-slot name="description">
            {{ __('filament-companies::companies.modal_descriptions.remove_company_employee') }}
        </x-slot>

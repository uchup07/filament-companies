<x-filament-companies::grid-section md="2">
    <x-slot name="title">
        {{ __('filament-companies::companies.grid_section_titles.profile_information') }}
    </x-slot>

    <x-slot name="description">
        {{ __('filament-companies::companies.grid_section_descriptions.profile_information') }}
    </x-slot>

    <x-filament::section>
        <x-filament-panels::form wire:submit="updateProfileInformation">
            <!-- Profile Photo -->
            @if (Uchup07\FilamentCompanies\FilamentCompanies::managesProfilePhotos())
                <div x-data="{ photoName: null, photoPreview: null }" class="space-y-2">
                    ...
                    <x-filament-forms::field-wrapper.label for="photo">
                        {{ __('filament-companies::companies.labels.photo') }}
                    ...
                    <x-filament::button size="sm" x-on:click.prevent="$refs.photo.click()">
                        {{ __('filament-companies::companies.buttons.new_photo') }}
                    </x-filament::button>

                    @if ($this->user->profile_photo_path)
                        <x-filament::button size="sm" color="danger" wire:click="deleteProfilePhoto">
                            {{ __('filament-companies::companies.buttons.remove_photo') }}
                        </x-filament::button>
                    @endif
            ...
            <!-- Name -->
            <x-filament-forms::field-wrapper id="name" statePath="name" required="required" label="{{ __('filament-companies::companies.fields.name') }}">
